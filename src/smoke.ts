...
export async function main(argv: string[] = process.argv.slice(2)): Promise<number> {
  ...
  async function sse(url: string, body: unknown, headers: Record<string, string> = {}) {
    ...
    if (text.includes("Could not load credentials")) {
      // the preflight above normally catches this; keep the guard for credential sources it cannot see
      throw new Error("no AWS credentials in the standard chain (AWS_PROFILE, AWS_ACCESS_KEY_ID/AWS_SECRET_ACCESS_KEY, AWS_BEARER_TOKEN_BEDROCK, ...)");
    }
