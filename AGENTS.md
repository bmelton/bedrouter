...
- Layout: `src/config.ts` (model map + cost), `src/translate.ts` (OpenAI <-> Converse, pure), `src/server.ts` (http routes, Bedrock calls, decision log). Run `npm test`, `npm run typecheck`, `npm run smoke`.
