...
| `GET /v1/sessions/:key` | | Running totals for one client session — every request that carried `x-bedrouter-session: <key>`, across conversations, including untracked ones and errors (requests, errors, conversations, cost, requested cost, classifier cost, tokens, escalations, per-route breakdown). Seeded from the decision log on startup, so a restart keeps the running session's numbers; `GET /v1/sessions` lists the last 50. pi-bedrouter sends Pi's session id and reads this for `/bedrouter usage` |
