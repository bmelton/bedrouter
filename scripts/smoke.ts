...
try {
  ...
  const c = await sse(`${base}/v1/chat/completions`, { model: openaiModel, max_tokens: 20, stream: true, messages: [{ role: "user", content: "Say hi in one word." }] });
  const cText = c.lines.filter((l) => l !== "[DONE]").map((l) => { try { return JSON.parse(l).choices?.[0]?.delta?.content ?? ""; } catch { return ""; } }).join("");
  const cErr = errorIn(c);
  console.log(`/v1/chat/completions [${openaiModel}] status=${c.status} text=${JSON.stringify(cText)}${cErr ? " error=" + cErr.message : ""}`);
